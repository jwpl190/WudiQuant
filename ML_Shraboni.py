...
for stock in stocks:
    ...
    if twice == True:
        ...
        data = w.wsd(stock, "open,high,low,close,volume,turn,MA,pct_chg", "2017-06-01", "2018-02-26", "MA_N=20;Fill=Previous;PriceAdj=F")
        data = convertWSDData(data)
        try:
         data.to_csv("C:/KeLiQuant/ztb/" + stock + "_Shraboni.csv", mode = 'a',index=True,
                    columns=["OPEN", "HIGH", "LOW", "CLOSE", "VOLUME", "TURN", "MA", "PCT_CHG"], header=True)
        except:
            continue
