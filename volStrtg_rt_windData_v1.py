...
##############################sell function######################
def sellFunc(stock, last, sellType,buy_left,sell_left,vol_last_trade_type):
    if sell_left[stock] > 0:
        sellable_quantity = daily_sellable[stock]
        trade_quantity = each_stock_trade_quantity[stock]
        trade_sell = min(sellable_quantity, trade_quantity)
        print("should sell ", str(trade_sell),', sell type: ', sellType)
        ...
        each_stock_position[stock] = each_stock_position[stock] - quantity
        daily_sellable[stock] = daily_sellable[stock] - quantity

    else:
        print("no more sell left")

##############################buy function######################
def buyFunc(stock, last, buyType, isDouble,buy_left,vol_last_trade_type):
    if buy_left[stock] > 0:
        if isDouble == True:
            trade_quantity = each_stock_trade_quantity[stock] * 2
        else:
            trade_quantity = each_stock_trade_quantity[stock]
        print(buyType)
...
def loadConfig():
    ...
    ####### get all stocks####################
    global stocks
    stocks = getAllStocks(stock_conf)
    number_of_stocks = len(stocks)
    for stock in stocks:
        each_stock_position[stock] = getStockPosition(stock, stock_conf)
        each_stock_trade_quantity[stock] = getStockEachTradeQuantity(stock,stock_conf)
        daily_sellable[stock] = getStockDailySellable(stock,stock_conf)
        if stock not in each_stock_cash:
            each_stock_cash[stock] = cash / number_of_stocks

        if stock not in first_time:
            isManualAdd = getIsManualAdd(stock, stock_conf)
...
cash = 1000000
each_stock_cash = {}
each_stock_trade_quantity = {}
each_stock_position = {}
daily_sellable = {}
number_of_stocks = 20
each_stock_last_price = {}
first_time = {}
...
def main(start=0):
    while 1:
        ...
        if weekno in [0, 1, 2, 3, 4, 5,6]:  # should be [0,1,2,3,4]


            # stocks = ["002049.SZ"]  #################need to delete@#################################################################
            date_time = datetime.today().strftime('%Y-%m-%d %H-%M')  # US today is Beijing yesterday
            today = date_time.split(' ')[0]
            #######For test only#########
            today = '2018-04-23'##Monday
            ###########################
            ...
            if curTime == '08-00':
                ...
                for stock in stocks:
                    ...
                    if each_stock_position[stock] == 0:
                        year_avg[stock] = w.wsd(stock, "MA", prev_t_day, prev_t_day, "MA_N=255;Fill=Previous;PriceAdj=F").Data[0][-1]
                        thirty_avg[stock] = w.wsd(stock, "MA", prev_t_day, prev_t_day, "MA_N=30;Fill=Previous;PriceAdj=F").Data[0][-1]
                        prev_19_day = getTDays(-19,prev_t_day)
                        price_highest_20[stock] = max(w.wsd(stock, "high", prev_19_day, prev_t_day, "Fill=Previous;PriceAdj=F").Data[0])
                    ###################Special zhi sun strategy##########################
                    if stock in special_zhisun_day.keys():
                        teshu_zhisun_day = str(special_zhisun_day[stock])
                        special_zhisun_price[stock] = w.wsd(stock, "MA", prev_t_day, prev_t_day, "MA_N=" + teshu_zhisun_day + ";Fill=Previous;PriceAdj=F").Data[0][-1]
                    else:
                        ...
                        last_close = w.wsd(stock, "close", prev_t_day, prev_t_day, "Fill=Previous;PriceAdj=F").Data[0][-1]#yesterday close
                        if (last_close - price_lowest_10) / price_lowest_10 > 0.3:
                            five_flag = False
                            ten_flag = False
                            twenty_flag = False
                            price_close_21_day = w.wsd(stock, "close", prev_20_day, prev_t_day, "Fill=Previous;PriceAdj=F").Data[0]
                            price_close_20_day = price_close_21_day[0:20]
                            five_avg_21_day = w.wsd(stock, "MA", prev_20_day, prev_t_day, "MA_N=5;Fill=Previous;PriceAdj=F").Data[0]
                            five_avg_20_day = five_avg_21_day[0:20]
                            for i in range(len(price_close_20_day)):
                                if price_close_20_day[i] < five_avg_20_day[i]:
                                    five_flag = True
                            if five_flag == True:
                                ten_avg_21_day = w.wsd(stock, "MA", prev_20_day, prev_t_day, "MA_N=10;Fill=Previous;PriceAdj=F").Data[0]
                                ten_avg_20_day = ten_avg_21_day[0:20]
                                for i in range(len(price_close_20_day)):
                                    if price_close_20_day[i] < ten_avg_20_day[i]:
                                        ten_flag = True
                                if ten_flag == True:
                                    twenty_avg_21_day = w.wsd(stock, "MA", prev_20_day, prev_t_day, "MA_N=20;Fill=Previous;PriceAdj=F").Data[0]
                    ...
                    price_close_vol = w.wsd(stock, "close", prev_backDays_tday, prev_t_day, "Fill=Previous;PriceAdj=F").Data[0]
                    ...
                    daily_sellable[stock] = each_stock_position[stock]  # each day sellable quantity for each stock
            ...
            ###########################################################trading#####################################################################
                ...
                for stock in stocks:
                    ...
                    last = wdata.Data[0][0]
                    quantity = each_stock_position[stock]
                    if quantity == 0:
                        # if zhisun happened, abandon this stock
                        if stock in zhisun_stock_temp:
                            updated_stocks.remove(stock)
                            zhisun_stock_temp.remove(stock)
                            continue
                        ...
                        if first_time[stock] == True:
                            ...
                            ####trade api###
                            each_stock_position[stock] = each_stock_position[stock] + quantity
                            each_stock_trade_quantity[stock] = quantity / 4  # volatility strategy trade volume
                        ...
                        isFixZhisun = getIsFixedZhisunPrice(stock,stock_conf)
                        if isFixZhisun == True:
                            zhisun_p = getZhisunPrice(stock, stock_conf)
                        else:
                            prev_t_day = getTDays(-1, today)
                            zhisun_day = getZhisunDay(stock, stock_conf)
                            zhisun_p = w.wsd(stock, "MA", prev_t_day, prev_t_day, "MA_N=" + zhisun_day + ";Fill=Previous;PriceAdj=F").Data[0][-1]
                        # fixed price zhisun
                        ...
                        if stock_exec_flag[stock] == True:
                            ...
                            if vol_up_open_flag[stock] == True and vol_down_open_flag[stock] == True:
                                ...
                                if range_index == 0:
                                    ...
                                    if vol_last_trade_type[stock] == 'sell2' or vol_last_trade_type[stock] == 'buy2' or vol_last_trade_type[
                                        ...
                                        if checkWeimai(stock) == True:
                                            print(stock, 'a')
                                            sellFunc(stock, last, 'sell1',buy_left,sell_left,vol_last_trade_type)
                                # price is at 2nd interval
                                    ...
                                    if vol_last_trade_type[stock] == 'sell3' or vol_last_trade_type[
                                        ...
                                        if checkWeimai(stock) == True:
                                            print(stock, 'b')
                                            sellFunc(stock, last, 'sell2',buy_left,sell_left,vol_last_trade_type)
                                # price is at 3rd interval
                                    ...
                                    if vol_last_trade_type[stock] == 'sell1':
                                        if checkWeimai(stock) == True:
                                            print(stock, 'c')
                                            buyFunc(stock, last, 'buy2', False,buy_left,vol_last_trade_type)
                                    # last trade was buy5,buy6  --> sell3
                                    elif vol_last_trade_type[stock] == 'buy5' or vol_last_trade_type[stock] == 'buy6':
                                        if checkWeimai(stock) == True:
                                            print(stock, 'd')
                                            sellFunc(stock, last, 'sell3',buy_left,sell_left,vol_last_trade_type)
                                # price is at 4th interval
                                    ...
                                    if vol_last_trade_type[stock] == 'sell1' or vol_last_trade_type[stock] == 'sell2':
                                        if checkWeimai(stock) == True:
                                            print(stock, 'e')
                                            buyFunc(stock, last, 'buy3', False,buy_left,vol_last_trade_type)
                                    # last trade was buy5,buy6  --> sell4
                                    elif vol_last_trade_type[stock] == 'buy5' or vol_last_trade_type[stock] == 'buy6':
                                        if checkWeimai(stock) == True:
                                            print(stock, 'f')
                                            sellFunc(stock, last, 'sell4',buy_left,sell_left,vol_last_trade_type)
                                # price is at 5th interval
                                    ...
                                    if vol_last_trade_type[stock] == 'sell1' or vol_last_trade_type[stock] == 'sell2':
                                        if checkWeimai(stock) == True:
                                            print(stock, 'g')
                                            buyFunc(stock, last, 'buy4', False,buy_left,vol_last_trade_type)
                                # price is at 6th interval
                                    ...
                                    if vol_last_trade_type[stock] == 'sell1' or vol_last_trade_type[
                                        ...
                                        if checkWeimai(stock) == True:
                                            print(stock, 'h')
                                            buyFunc(stock, last, 'buy5', False,buy_left,vol_last_trade_type)
                                # price is at 7th interval
                                    ...
                                    if vol_last_trade_type[stock] == 'sell1' or vol_last_trade_type[
                                        ...
                                        if checkWeimai(stock) == True:
                                            print(stock, 'i')
                                            buyFunc(stock, last, 'buy6', True,buy_left,vol_last_trade_type)
                            # up intervals open
                            ...
                            # down intervals open
                                ...
                                # price is at 3rd interval
                                    ...
                                    if vol_last_trade_type[stock] == 'buy6':
                                        if checkWeimai(stock) == True:
                                            print(stock, 'q')
                                            sellFunc(stock, last, 'sell5',buy_left,sell_left,vol_last_trade_type)
                            ...
                            # all intervals close
                                ...
                                if last >= stock_vol[stock][0]:
                                    if checkWeimai(stock) == True:
                                        ...
                                        sellFunc(stock, last, 'sell2',buy_left,sell_left,vol_last_trade_type)
                                        vol_up_open_flag[stock] = True
                                        updateRangesFromUp(stock_vol_abs[stock], last * 0.998, stock,stock_vol_range_all,stock_vol_range_up,stock_vol_range_down)
                                        print("update 2")
                                ...
                                # below 5th line,open down interval
                                    ...
                                    if checkWeimai(stock) == True:
                                        if (last - zhisun_p) / zhisun_p >= 0.02:
                                            ...
                                            buyFunc(stock, last, 'buy5', False,buy_left,vol_last_trade_type)
                                            vol_down_open_flag[stock] = True
                                            updateRangesFromDown(stock_vol_abs[stock], last * 1.002, stock,stock_vol_range_all,stock_vol_range_up,stock_vol_range_down)
                                            print("update 3")
                                            print(stock_vol_range_all)
                                            print(stock_vol_range_up)
                                            print(stock_vol_range_down)
                    fields = ["Position","EachStockTradeQuantity","StockDailySellable"]
                    values = [each_stock_position[stock],each_stock_trade_quantity[stock],daily_sellable[stock]]
                    updateConfig(stock,fields,values)
                global stocks
